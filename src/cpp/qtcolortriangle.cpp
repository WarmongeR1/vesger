...
void QtColorTriangle::mouseMoveEvent(QMouseEvent *e)
{
  /*  if ((e->buttons() & Qt::LeftButton) == 0)
    ...
    if (selMode == SelectingHue) {
        ...
        if (curHue != h) {
            newColor = true;
            curColorBack.setHsv(curHue, s, v);
        }
        ...
        selectorPos = pointFromColor(curColorBack);
    } else {
        Vertex aa(Qt::black, pa);
        Vertex bb(Qt::black, pb);
        Vertex cc(Qt::black, pc);
...
// отвечает за выбор цвета, когда человек зажимает пкм или лкм и водил по треугольнику, цвет сразу выбирается
    if (newColor)
     {if (e->button() != Qt::LeftButton)
       ...
       {
        emit colorChangedBack(curColorBack);
       }
...
void QtColorTriangle::setColorBack(const QColor &col)
{
    if (col == curColorBack)
        return;
...
QColor QtColorTriangle::colorBack() const
{
    return curColorBack;
}
/*!
